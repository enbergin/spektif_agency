...
export class BoardsService {
  ...
  async findOne(id: string, userId: string) {
    ...
    const orgMember = board.organization.members[0];
    if (!orgMember) {
      throw new ForbiddenException('Not a member of this organization');
    }
  ...
  async create(dto: CreateBoardDto, userId: string) {
    ...
    await this.prisma.list.createMany({
      data: [
        { boardId: board.id, title: 'Yapılacak', order: 1 },
        { boardId: board.id, title: 'Devam Ediyor', order: 2 },
        { boardId: board.id, title: 'Bitti', order: 3 },
      ],
    });
  ...
  async reorderLists(boardId: string, listOrders: { id: string; order: number }[], userId: string) {
    // Check board access
    const boardMember = await this.prisma.boardMember.findUnique({
      where: {
        boardId_userId: {
